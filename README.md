...
```
● 我扫描了你全年 365 天的账单。这是把你的一年铺成格子的样子……   ← 实时行：镜像 agent 正在说的话 / 思考句 / 工具调用
◌ 2.0s   ⌘ account_bills ✓                                          ← 轨迹：本章已完成的步骤，新章清空
● 已连接 · 只读            /okx-year-in-review   [过程 ⌃]            ← 状态条 + 展开开关
```

点「过程」或实时行展开完整对话流（思考块、工具调用 JSON、全部叙述）。需要用户操作的节点不强制展开：分支选择渲染为三张决策卡（编号 / 字符图标 / 标题 / 数据提示），折叠态直接出现在实时行下方，展开态在对话流里用同一套卡片并联动；「生成卡片」确认同理，折叠态下就是两颗按钮。
...
- **动效**：三层——ASCII 场（章节切换扫掠、解析时 storm）、粒子（每章一个专属 3D 形态、指针推挤、终章聚成品牌方块）、
  文字（解密揭示 / 打字机 / 滚数）。尊重 `prefers-reduced-motion`。
- **粒子形态**（`src/fx/particles.js` 的 `SHAPES`，12000 点，深绿 → 亮绿 → 近白三段明暗）：
  会话 三叶结 · 起点 螺旋星系 · 首笔 种子 → 参数玫瑰（花瓣轮廓线采样 + 花茎） · 活跃度 365 天柱状日历（真实数据）·
  最佳交易 信号尖峰 · 收获 线框宝石 · 交易量 三柱（真实占比）· 赚币 螺旋阶梯 · 机器人 4³ 线框格 · 峰值 山脉地形 ·
  资金流 无限环（进亮出暗）· DNA 双螺旋 · 人格 品牌方块。每个形态自带俯仰角、自旋速度、点径；`registerShape` 可注入数据驱动形态。
  玫瑰另有一层 48000 点的高密度「绽放层」（`BLOOMS`，高斯软点、加法混合、按花瓣由内向外展开），只在该形态出现，随主粒子同步旋转与交互。
